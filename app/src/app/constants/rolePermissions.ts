...
export const ROLE_PERMISSIONS: Record<RoleEnum, DrawerSection[]> = {
  ...
  // Kitchen - Órdenes y pantallas de preparación
  [RoleEnum.KITCHEN]: [
    'OrdersStack',
    'PreparationScreensStack',
    'AvailabilityStack',
...
export const hasPermission = (roleId: number | undefined, section: DrawerSection): boolean => {
...
export const DRAWER_SECTIONS = {
  sales: {
    title: 'Ventas',
    items: [
      { route: 'OrdersStack', label: 'Órdenes', icon: 'clipboard-list-outline' },
      { route: 'ReceiptsStack', label: 'Recibos', icon: 'receipt' },
      { route: 'OrderFinalizationStack', label: 'Finalización', icon: 'clipboard-check-outline' },
    ],
  },
  configuration: {
    title: 'Configuración',
    items: [
      ...
      { route: 'AvailabilityStack', label: 'Disponibilidad', icon: 'eye-off-outline' },
      { route: 'ModifiersStack', label: 'Modificadores', icon: 'tune' },
      { route: 'PizzaCustomizationsStack', label: 'Personalización Pizzas', icon: 'pizza' },
      { route: 'PreparationScreensStack', label: 'Pantallas Preparación', icon: 'monitor-dashboard' },
      { route: 'AreasTablesStack', label: 'Áreas y Mesas', icon: 'map-marker-radius-outline' },
      { route: 'PrintersStack', label: 'Impresoras', icon: 'printer' },
      { route: 'RestaurantConfigStack', label: 'Configuración', icon: 'cog-outline' },
      { route: 'CustomersStack', label: 'Clientes', icon: 'account-group-outline' },
