...
import { Modal, Portal, Surface, Text, Button, Divider, IconButton } from 'react-native-paper';
import { OrderForFinalization, OrderItemForFinalization } from '../types/orderFinalization.types';
...
export const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({
  ...
  const groupedItems = React.useMemo(() => {
    ...
    order.orderItems.forEach(item => {
      ...
      const modifierKeys = (item.modifiers && item.modifiers.length > 0) 
  ...
  const renderGroupedItem = ({ item, quantity }: GroupedItem) => {
    ...
    return (
      <Surface key={`${item.id}-${quantity}`} style={styles.itemCard} elevation={0}>
        ...
        {item.modifiers && item.modifiers.length > 0 && (
          <View style={styles.modifiers}>
            {item.modifiers.map(modifier => (
              <Text 
                key={modifier.id}
                style={[styles.modifierText, { color: theme.colors.onSurfaceVariant }]}
              >
                + {modifier.name} (${(typeof modifier.price === 'string' ? parseFloat(modifier.price) : modifier.price).toFixed(2)})
              </Text>
            ))}
          </View>
        )}
