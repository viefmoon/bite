...
export function ServerSettingsScreen() {
  ...
  const { isConnected, isHealthy, serverUrl, isSearching } = useServerConnection();
  ...
  const loadSettings = async () => {
    try {
      const [savedMode, savedUrl] = await Promise.all([
        EncryptedStorage.getItem(STORAGE_KEYS.CONNECTION_MODE) as Promise<ConnectionMode>,
      ...
      if (mode === 'auto') {
        ...
        try {
          ...
          if (discoveredUrl) {
            ...
            const { healthMonitoringService } = await import('@/services/healthMonitoringService');
        ...
        try {
          ...
          try {
            ...
            if (isHealthy) {
              showSnackbar('Configuración guardada - Servidor accesible ✓', 'success');
              healthMonitoringService.startMonitoring();
            } else {
              showSnackbar('Configuración guardada - El servidor no responde ✗', 'warning');
            }
          } catch (error) {
            showSnackbar('Configuración guardada - Error al verificar servidor', 'warning');
          }
        }
      }
          ...
          <Surface style={styles.infoCard} elevation={0}>
            <View style={styles.infoRow}>
              <View style={styles.infoItem}>
                <Icon source="server" size={20} color={theme.colors.onSurfaceVariant} />
                <View style={styles.infoContent}>
                  <Text variant="labelSmall" style={styles.infoLabel}>Servidor</Text>
                  <Text variant="bodyMedium" style={styles.infoValue}>
                    {serverUrl ? serverUrl.replace(/^https?:\/\//, '').split(':')[0] : '-'}
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.infoItem}>
                <Icon source="ethernet-cable" size={20} color={theme.colors.onSurfaceVariant} />
                <View style={styles.infoContent}>
                  <Text variant="labelSmall" style={styles.infoLabel}>Puerto</Text>
                  <Text variant="bodyMedium" style={styles.infoValue}>
                    {serverUrl ? (serverUrl.split(':').pop()?.split('/')[0] || '3737') : '3737'}
                  </Text>
                </View>
              </View>
            </View>
            ...
            {serverUrl && !isHealthy && isConnected && (
              <View style={styles.healthWarning}>
                <Icon source="alert" size={16} color={theme.colors.warning || '#FF9800'} />
      ...
      <View style={styles.section}>
        ...
        >
          <TouchableRipple
            ...
            style={[styles.radioOption, mode === 'auto' && styles.radioOptionActive]}
          ...
          >
            <View style={styles.radioContent}>
              <Icon
                source="wifi"
                size={24}
                color={mode === 'auto' ? theme.colors.primary : theme.colors.onSurfaceVariant}
              />
          ...
          <TouchableRipple
            ...
            style={[styles.radioOption, mode === 'manual' && styles.radioOptionActive, { marginBottom: 0 }]}
            rippleColor={`${theme.colors.primary}20`}
          >
            <View>
              <View style={styles.radioContent}>
                <Icon
                  ...
                  size={24}
                  color={mode === 'manual' ? theme.colors.primary : theme.colors.onSurfaceVariant}
                />
                <View style={styles.radioTextContainer}>
                  <Text variant="titleMedium" style={[
                    styles.radioTitle,
              ...
              {mode === 'manual' && (
                <View style={styles.manualInputContainer}>
                  ...
                  />
                  <HelperText 
                    type={manualUrl !== '' && !validateUrl(manualUrl) ? 'error' : 'info'} 
                    visible={true}
                  >
        ...
        <View style={styles.discoveryProgressContainer}>
          <Surface style={styles.discoveryProgressCard} elevation={1}>
            <View style={styles.discoveryHeader}>
              <Icon source="magnify-scan" size={24} color={theme.colors.primary} />
              <Text style={styles.discoveryTitle}>Buscando servidor en la red</Text>
            </View>
            <ProgressBar 
              progress={discoveryProgress.total > 0 ? discoveryProgress.current / discoveryProgress.total : 0}
            ...
            {discoveryProgress.total > 0 && (
              <Text style={styles.discoveryStats}>
                {discoveryProgress.current} de {discoveryProgress.total} IPs escaneadas
              </Text>
            )}
          </Surface>
        </View>
        ...
        <Dialog visible={showInfo} onDismiss={() => setShowInfo(false)}>
          ...
          <Dialog.Content>
            <Paragraph style={styles.infoParagraph}>
              ...
              {'\n'}Ideal para uso en el restaurante. La app busca el servidor en la red local.
            </Paragraph>
            
            
            <Paragraph style={styles.infoParagraph}>
              <Text style={styles.boldText}>Modo Manual:</Text>
              {'\n'}Configura manualmente la URL del servidor para casos especiales.
            </Paragraph>
          </Dialog.Content>
          <Dialog.Actions>
...
const createStyles = (theme: any, responsive: any) => StyleSheet.create({
  container: {
    flex: 1,
