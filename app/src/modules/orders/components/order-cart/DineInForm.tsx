...
export const DineInForm = forwardRef<DineInFormRef, DineInFormProps>((
  ...
  const { data: tablesData = [], isLoading: isLoadingTables, error: errorTables } = useGetTablesByArea(
    selectedAreaId || '',
    { enabled: !!selectedAreaId }
  );
  const selectedAreaName = areasData.find((a: any) => a.id === selectedAreaId)?.name || 'Selecciona un área';
  const selectedTableName = tablesData.find((t: Table) => t.id === selectedTableId)?.name || 'Selecciona una mesa';
  const formattedScheduledTime = scheduledTime ? format(scheduledTime, 'h:mm a').toLowerCase() : '';
