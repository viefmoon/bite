...
const formatValue = (field: string, value: any): string => {
  ...
  if (field === 'total' || field === 'subtotal' || field === 'basePrice' || field === 'finalPrice') {
...
const HistoryItem: React.FC<{ item: OrderHistory; theme: ReturnType<typeof useAppTheme> }> = ({ item, theme }) => {
  ...
  const getChangeSummary = () => {
    ...
    if (item.operation === 'UPDATE' && item.diff) {
      ...
      for (const [field, change] of Object.entries(item.diff || {})) {
        ...
        } else if (field === 'orderStatus' && Array.isArray(change) && change.length >= 2) {
          changes.push('Estado cambiado');
        } else if (field === 'orderItems' && typeof change === 'object') {
          changes.push('Productos modificados');
        } else if (field === 'customerName' || field === 'phoneNumber' || field === 'deliveryAddress') {
  ...
  const renderChanges = () => {
    ...
    if (item.operation === 'UPDATE' && item.diff) {
      ...
      for (const [field, change] of Object.entries(item.diff || {})) {
          ...
          for (const [key, value] of Object.entries(change as any)) {
            ...
            if (!isNaN(parseInt(key))) {
              ...
              if (Array.isArray(value) && value.length === 1 && typeof value[0] === 'object') {
                ...
                productDetails.push(
                  <View key={`item-${key}`} style={styles.changeRow}>
                    <Text style={[styles.changeLabel, styles.removedLabel]}>Eliminado:</Text>
                    <Text style={[styles.changeValue, styles.removedText]}>{productName}</Text>
                  </View>
                );
                ...
                if ('quantity' in value && Array.isArray(value.quantity)) {
                  itemChanges.push(`Cantidad: ${value.quantity[0]} → ${value.quantity[1]}`);
                }
                if ('finalPrice' in value && Array.isArray(value.finalPrice)) {
                  itemChanges.push(`Precio: $${value.finalPrice[0]} → $${value.finalPrice[1]}`);
                }
        ...
        } else if (Array.isArray(change) && change.length === 3 && change[1] === 0 && change[2] === 0) {
          // Campo eliminado
          const oldValue = formatValue(field, change[0]);
          changes.push(
            <View key={field} style={styles.changeRow}>
              <Text style={[styles.changeLabel, styles.removedLabel]}>{formatFieldName(field)}:</Text>
...
export const OrderHistoryModal: React.FC<OrderHistoryModalProps> = ({
  ...
  const allHistoryItems = useMemo(() => {
    ...
    return orderHistoryData.pages.flatMap(page => (page as { data: OrderHistory[] })?.data || []);
  }, [orderHistoryData]);

  const renderHistoryItem = useCallback(({ item }: { item: OrderHistory }) => {
        ...
        <View style={styles.container}>
          ...
          {!orderId ? (
            ...
            <View style={styles.errorContainer}>
              ...
              <Button onPress={() => refetch()} mode="text">Reintentar</Button>
            </View>
          ) : (
            <FlatList
